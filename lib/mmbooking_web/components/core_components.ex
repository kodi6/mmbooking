defmodule MmbookingWeb.CoreComponents do
  @moduledoc """
  ...
  def custom_input(%{type: "checkbox"} = assigns) do
    assigns =
      assign_new(assigns, :checked, fn ->
        Phoenix.HTML.Form.normalize_value("checkbox", assigns[:value])
      end)
  ...
  def custom_input(%{type: "select"} = assigns) do
    ~H"""
    <div phx-feedback-for={@name}>
  ...
  def custom_label(assigns) do
    ~H"""
    <label for={@for} class="block  font-semibold leading-6 text-zinc-800 w-[9rem]">
      <%= render_slot(@inner_block) %>
    </label>
    """
  end
...
error: undefined function sigil_p/2 (expected MmbookingWeb.Nav to define such a function or for it to be imported, but none are available)
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(MmbookingWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(MmbookingWeb.Gettext, "errors", msg, opts)
    end
  end
