defmodule MmbookingWeb.SessionLive.Index do
  ...
  def handle_params(%{"template_id" => template_id}, _url, socket) do
          ...
          |> redirect(to: ~p"/sessions/?template_id=#{template.id}")}
  ...
  def handle_event("template", %{"name" => name} = params, socket) do
  ...
  defp create_template(name) do
    template = Templates.create_template(%{"name" => name})

    sessions = if String.contains?(name, "darshan") do
      Sessions.create_darshan_session(template.id)
    else
      Sessions.create_default_session(template.id)
    end
